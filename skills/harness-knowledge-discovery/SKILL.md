...
### 7. Create and configure post-task hook

Create the hook script that marks task completion in the execution log. Without this hook, the feedback loop never triggers and memory files stay empty forever.

**For Cursor projects** (`.cursor/hooks.json` exists):
...
Register it in `.cursor/hooks.json` under `beforeSubmitPrompt`. If `jq` is available:
```bash
jq '.hooks.beforeSubmitPrompt += [{"command": ".cursor/hooks/harness-post-task.sh"}]' .cursor/hooks.json > .cursor/hooks.json.tmp && mv .cursor/hooks.json.tmp .cursor/hooks.json
```

If `jq` is not available, manually append the hook entry to the `beforeSubmitPrompt` array in `.cursor/hooks.json`.

**For Claude Code projects** (`.claude/` directory exists but no hooks.json):

Add a reminder to `CLAUDE.md`:
```markdown
## Reminder
At the end of each task, run the `harness-feedback-loop` skill to collect execution evidence.
```

**If both exist**: configure both.
**If neither exists**: skip — feedback loop must be invoked manually.
...
- **Not creating post-task hook** — without a hook, the feedback loop never triggers automatically; failure/success patterns stay empty forever
